...
public class AuthenticationFilterWarden implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (
                uri.endsWith("/warden/warden-login.jsp")
                        || uri.contains("/warden/css/")
                        || uri.contains("/warden/js/")
                        || uri.contains("/warden/image/")) {
        ...
        if (session == null || session.getAttribute("current_warden") == null) {
            res.sendRedirect(req.getContextPath() + "/warden/warden-login.jsp");
            return;
        }
        // User authenticated
        chain.doFilter(request, response);
